...
│   ├── error/
│   ├── platform/
│   │   └── sdk_channel.dart      # MethodChannel 래퍼
│   └── utils/
├── data/                         # Data 레이어
│   ├── datasources/
│   │   ├── local/                # SharedPreferences 접근
│   │   └── remote/               # 네이티브 SDK 접근
│   ├── models/
│   │   └── *_api_model.dart      # API/SDK 응답 모델
│   ├── repositories/
│   └── mappers/
├── domain/                       # Domain 레이어
│   ├── entities/
│   │   └── *_entity.dart         # 비즈니스 모델
│   ├── repositories/
│   │   └── *_repository.dart     # 인터페이스 정의
│   └── usecases/
